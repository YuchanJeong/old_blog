...
우선 `차세대 형식을 사용해 이미지 제공하기`를 따라 이미지를 webp로 모두 변경했다. 그리고 `이미지 크기 적절하게 설정하기`를 따라 실제 이미지의 크기도 적절하게 줄였다. 또 \<Image /> 요소의 크기도 명시적으로 표시해 줬다. 그 결과 성능이 눈에띄게 개선 되었다. (성능 30점 상승 및 17.56초 개선)
