...
export const usePostDebate = (
  ...
  return useMutation(() => postDebate(debate), {
    onSuccess: (debateId) => {
...
export const usePatchDebate = (
  ...
  const router = useRouter();
  const queryClient = useQueryClient();
  return useMutation(() => patchDebate(debate), {
    onSuccess: (debateId) => {
...
export default function Edit() {
  ...
  const prosConsRadio = useRadio(
    data?.author_pros ? `${data?.author_pros}` : "true",
    "prosCons"
  );
  const contentsInput = useInput(data?.title || "", "");
  ...
  const postDebate = usePatchDebate(debate, setIsErrorModalOn, true);
