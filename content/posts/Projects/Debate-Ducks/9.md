...
재연결 시 녹화의 안정적인 유지를 위해 서버에 Blobs를 저장하였는데 최종적으로 AWS의 S3에 저장되기 때문에 서버를 거치는 것이 비효율적인 방법이라고 생각해 수정할 예정이다. (클라이언트에서 바로 처리하면서도 안정성을 높일 방법 고안 필요)

### 녹화 영상 깨짐 현상

토론이 중단된 후 재시작 되었을 때 재시작 이후의 녹화 영상이 깨지는 현상이 발생한다.
...
재연결 과정에서 녹화의 대상이 변경되기 때문에 발생하는 문제라 추측되나 현제의 재연결 기능에서는 확인하기 어려워 수정할 예정이다.
