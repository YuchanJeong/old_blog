...
{{< alert "circle-info" >}}
당장 취업 준비해야 하는데 만들고 싶은 게 생겨버려서 진행하는 프로젝트!
{{< /alert >}}
...
우선 코드의 안전성을 위해서 `TypeScript`를 사용할 것이다. 다음으로 시험 문제는 한번 만들어지면 변경되지 않을 것이라 SSG를 사용하기 적합하고, 이를 위해 `Next.js`를 사용할 것이다. 상태 관리는 코드의 유지 보수성을 위해 `Redux`를 사용할 것이다. 마지막으로 JS의 상수와 함수를 스타일에도 활용하기 위해 `Styled Components`를 사용할 것이다.

데이터 베이스는 NoSQL이면서 서버리스로 사용할 수 있는 AWS의 DynamoDB를 사용하기로 했다. [[공부 시작]](/posts/studies/db/dynamodb/1/)

## 와이어 프레임 - main

진짜 시험지 느낌이 나도록 촌스럽고 딱딱하게 만드는 것이 목표이다.
